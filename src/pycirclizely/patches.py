...
class PolarSVGPatchBuilder:
    ...
    def bezier_ribbon_path(
        cls,
        rad1_start: float, rad1_end: float, r1: float,
        rad2_start: float, rad2_end: float, r2: float,
        height_ratio: float,
        direction: int = 0,
        ...
        if direction in (-1, 2):
            ...
            path = f"M {start_point[0]},{start_point[1]}"
            path += " " + cls.arc_line((rad1_start, rad1_end), (r1, r1)).split(" ", 1)[1]
        
        # Append Bezier curve to region2 (end1 → end2)
        path += " " + cls._bezier_segment(
            rad1_end, r1,
            rad2_end, arrow_r2 if direction in (1, 2) else r2,
            height_ratio
        )
        ...
        path += " " + cls._bezier_segment(
            rad2_start, arrow_r2 if direction in (-1, 2) else r2,
            rad1_start, arrow_r1 if direction in (-1, 2) else r1,
            height_ratio
        )
    ...
    def _arrow_path(cls, rad_start: float, rad_end: float, r_base: float, r_tip: float) -> str:
        ...
        return (f" L {p_tip[0]},{p_tip[1]}"    # Draw to tip
                f" L {p_end[0]},{p_end[1]}")   # Close arrow